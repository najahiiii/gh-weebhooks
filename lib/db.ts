...
function resolveSqlitePath(url: string): string {
  if (url.startsWith("sqlite://")) {
    // Keep absolute paths intact; resolve relative paths against the project root.
    const target = url.startsWith("sqlite:///") ? url.replace("sqlite:///", "") : url.replace("sqlite://", "");
    if (path.isAbsolute(target)) {
      return target;
    }
    const baseDir = path.basename(process.cwd()) === "server" ? path.resolve(process.cwd(), "..") : process.cwd();
